 ...
 *
 * The argument should be in the form: Recipepress\Namespace. The
...
spl_autoload_register( static function( $class_name ) {
	...
	if ( false === strpos( $class_name, 'wzy' ) ) {
		return;
	}
	...
	for ( $i = count( $file_parts ) - 1; $i > 0; $i-- ) {
		...
		if ( count( $file_parts ) - 1 === $i ) {
			/**
			 * If 'Interfaces' or 'Traits' is contained in the parts of the file name, then
			 * define the $file_name differently so that it's properly loaded.
			 * Otherwise, just set the $file_name equal to that of the class
			 * filename structure.
			 */
			if ( in_array( 'Interfaces', $file_parts, true ) ) {
			...
			} elseif ( in_array( 'Traits', $file_parts, true ) ) {
				// Grab the name of the interface from its qualified name.
				$trait_name = explode( '_', $file_parts[ count( $file_parts ) - 1 ] );
				$trait_name = strtolower( implode( '-', $trait_name ) );
	...
	$file_path  = trailingslashit( untrailingslashit( plugin_dir_path( dirname( __DIR__ ) ) ) . $namespace );
	$file_path .= $file_name;
